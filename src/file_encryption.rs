...
pub fn encrypt_file<'py>(
    ...
    let _salt_array: [u8; 16] = salt
        .try_into()
        .map_err(|_| PyValueError::new_err("Salt must be 16 bytes"))?;
    let data = fs::read(input_path).map_err(|e| PyIOError::new_err(e.to_string()))?;
    let encrypted_data = encrypt(py, &data, &key_array)?;
    fs::write(output_path, encrypted_data.as_bytes())
